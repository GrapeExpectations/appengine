...
func Get(ctx context.Context, key *datastore.Key, dst interface{}) error {
	if key == nil {
		return errors.New(http.StatusBadRequest, errors.Message{Pkg: "datastore", Fn: "Get", Msg: "key not provided"})
	}
	...
	if !reflect.PtrTo(elemType).Implements(iEntity) {
		return errors.New(http.StatusBadRequest, errors.Message{Pkg: "datastore", Fn: "Get", Msg: "type does not implement Entity interface"})
	}
